...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uniRequest("POST", "get_Chinese_athletes", null, "application", res => {
		...
		methods: {
			...
			search() {
				...
				uniRequest("POST", "find_athlete_by_name", data, "application", res => {
					this.athletes = res.data.athletes;
					wx.hideToast();
				});
			},
			...
			change(index) {
				...
				if (index == 0) {
					...
					uniRequest("POST", "get_foreign_athletes", null, "application", res => {
						this.athletes = res.data.athletes;
						wx.hideToast();
					});
				} else if (index == 2) {
					...
					uniRequest("POST", "get_male_athletes", null, "application", res => {
						this.athletes = res.data.athletes;
						wx.hideToast();
					});
				} else if (index == 3) {
					...
					uniRequest("POST", "get_female_athletes", null, "application", res => {
						this.athletes = res.data.athletes;
						wx.hideToast();
					});
				} else if (index == 4) {
					...
					uniRequest("POST", "get_athletes", null, "application", res => {
						this.athletes = res.data.athletes;
						wx.hideToast();
					});
				};
