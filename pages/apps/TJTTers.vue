...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uniRequest("POST", "update_TJTTers", null, "application", res => {
				console.log(res.data.message);
			});
			uniRequest("POST", "get_TJTTers", null, "application", res => {
		...
		methods: {
			setAbtn(e) {
				...
				if (e == 0) {
					...
					uniRequest("POST", "get_dept_technology_TJTTers", null, "application", res => {
						this.TJTTers = res.data.TJTTers;
						wx.hideToast();
					});
				}
					...
					uniRequest("POST", "get_dept_organizing_TJTTers", null, "application", res => {
						this.TJTTers = res.data.TJTTers;
						wx.hideToast();
					});
				}
					...
					uniRequest("POST", "get_dept_culture_TJTTers", null, "application", res => {
						this.TJTTers = res.data.TJTTers;
						wx.hideToast();
					});
				};
