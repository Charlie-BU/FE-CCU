	...
	<div>
		<section class="section">
			...
			<div v-for="(player, index) in this.players" :key="index" class="boxList flex">
				<div class="left">
					<img class="lIcon"
						src="https://ccu-assets.oss-cn-beijing.aliyuncs.com/images/unlogined.png"
						alt="" />
				</div>
				<div class="right">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uniRequest("POST", "get_this_match", {"match_id": match_id}, "competition", res => {
				this.match = res.data.match;
			});
			
			// 获取参赛选手
			uniRequest("POST", "get_match_players", {"match_id": match_id}, "competition", res => {
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "add_player", data, "competition", res => {
								wx.showToast({
								...
								if (res.data.status == 200) {
									uniRequest("POST", "get_match_players", {"match_id": this.match.id}, "competition", res => {
										this.players = res.data.players;
									});
								}
							})
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "reset_present", data, "competition", res => {
								wx.showToast({
									title: res.data.message,
									icon: "none",
									duration: 1000,
					...
					wx.showModal({
						...
						success: res => {
							if (res.confirm) {
								...
								uniRequest("POST", "player_sign_or_unsign", data, "competition", res => {
									wx.showToast({
										title: res.data.message,
										icon: "none",
										duration: 1000,
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "remove_player", data, "competition", res => {
								wx.showToast({
									title: res.data.message,
									icon: "none",
									duration: 1000,
