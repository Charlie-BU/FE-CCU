	...
	<div>
		...
		<section class="section">
			...
			<div class="box2 flex" style='margin-top: 30rpx;'>
				<div class="boxT" @click="equipment_page">
					<div class="tp flex">
						<div class="LEFT">
			...
			<div class="box2 flex" style='margin-top: 30rpx;'>
				<div class="boxT" @click="TJTTers_page">
					<div class="tp flex">
						<div class="LEFT">
							<img class="LEFTiCON round" src="https://ccu-assets.oss-cn-beijing.aliyuncs.com/images/logo.jpg" alt="" />
						</div>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			// 获取赛季信息
			uniRequest("POST", "get_this_season", null, "competition", res => {
				this.this_season = res.data.this_season;
				const data = {
					'my_id': this.user.id,
					'season_id': this.this_season.id,
				};
				uniRequest("POST", "get_user_season_recording", data, "user", res => {
				...
				uniRequest('POST', 'cal_active', { 'user_id': this.user.id }, 'user', res => {
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "send_feedback", data, "user", res => {
								wx.showToast({
...
<style lang="scss" scoped>
	...
	.box2 {
		...
		.boxT {
			box-sizing: border-box;
			padding: 20rpx;
			background: linear-gradient(180deg, #1ABFC0, #f9d5a5);
			width: 48%;
			border-radius: 20rpx;
			display: flex;
			align-items: center;
