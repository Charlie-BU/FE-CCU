...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uniRequest("POST", "match_list_current", null, "competition", res => {
		...
		methods: {
			...
			search() {
				...
				uniRequest("POST", "findMatchByName", data, "competition", res => {
					this.matches = res.data.matches;
					wx.hideToast();
				});
			},
			...
			change(index) {
				...
				if (index == 0) {
					...
					uniRequest("POST", "match_list_signed", {"my_id": this.user.id}, "competition", res => {
						this.matches = res.data.matches;
						wx.hideToast();
					});
				} else if (index == 2) {
					...
					uniRequest("POST", "match_list_history", null, "competition", res => {
						this.matches = res.data.matches;
						wx.hideToast();
					});
				} else if (index == 3) {
					...
					uniRequest("POST", "match_list_all", null, "competition", res => {
						this.matches = res.data.matches;
						wx.hideToast();
					});
				};
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "quit_match", data, "competition", res => {
								wx.showToast({
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "hide_match", data, "competition", res => {
								wx.showToast({
									title: res.data.message,
									icon: "none",
									duration: 1000,
