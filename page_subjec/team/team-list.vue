	...
	<div class="app">
		...
		<section class="section">
			<div style="display: flex;">
				...
				<button class="cu-btn btn round line-cyan" style="margin: 8rpx 20rpx;" @click="add_team">+ 添加代表队</button>
			</div>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uniRequest("POST", "get_teams", null, "competition", res => {
		...
		methods: {
			...
			search() {
				...
				uniRequest("POST", "findTeamByName", data, "competition", res => {
					this.teams = res.data.teams;
					this.team_length = res.data.team_length;
				});
			},
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "modify_member_max", data, "competition", res => {
								wx.showToast({
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "modify_match", data, "competition", res => {
								wx.showToast({
									title: res.data.message,
									icon: "none",
									duration: 1000,
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "delete_team", data, "competition", res => {
								wx.showToast({
									title: res.data.message,
									icon: "none",
									duration: 1000,
