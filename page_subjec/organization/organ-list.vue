...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uniRequest("POST", "get_info", null, "organization", res => {
				this.organ_length = res.data.organ_length;
				this.tongji_length = res.data.tongji_length;
			});
			// 获取组织
			uniRequest("POST", "get_organs", null, "organization", res => {
		...
		methods: {
			...
			search() {
				...
				uniRequest("POST", "findOrganByName", data, "organization", res => {
					this.organs = res.data.organs;
				});
			},
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "delete_organ", data, "organization", res => {
								wx.showToast({
