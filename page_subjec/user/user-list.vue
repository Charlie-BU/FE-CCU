	...
	<div class="app">
		...
		<section class="section">
			<div style="display: flex;">
				...
				<button class="cu-btn btn round line-cyan" style="margin: 8rpx 20rpx;" @click="add_user">+ 添加用户</button>
			</div>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			// 获取用户
			uniRequest("POST", "get_users_by_join_time", null, "user", res => {
		...
		methods: {
			...
			search() {
				...
				uniRequest("POST", "findUserByName", data, "user", res => {
					this.users = res.data.users;
				});
			},
				...
				wx.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							uniRequest("POST", "modify_score", data, "user", res => {
								wx.showToast({
